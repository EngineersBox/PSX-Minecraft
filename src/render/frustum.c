...
//             /*    .distance = 0*/
//             /*},*/
//             [FRUSTUM_PLANE_NEAR] = (Plane) { .normal = vec3_i32(0, 0, -ONE), .distance = 409, .point = vec3_i32(0, 0, 409) },
//             [FRUSTUM_PLANE_FAR] = (Plane) { .normal = vec3_i32(0, 0, ONE), .distance = 10 * ONE, .point = vec3_i32(0, 0, ONE) },
...
FrustumQueryResult conicFrustumContainsAABB(const Camera* camera,
    ...
    const VECTOR u = rotationToDirection(&camera->rotation);
    const VECTOR v = *aabbCentre;
    // TODO: Complete this
}
